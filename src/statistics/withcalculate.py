...
# Create a statistical object and reserve only the time and score
game_stats = {
    'time': [],
    'scores': []
}
...
def save_game_stats():
    try:
        with open('/Users/stella/Desktop/game_stats.csv', 'w', newline='') as f:
            ...
            for i in range(len(game_stats['time'])):
                writer.writerow([
                    game_stats['time'][i],
                    game_stats['scores'][i]
                ])
        print("Game data has been saved to: /Users/stella/Desktop/game_stats.csv")
...
rewards = []
reward_timer = 0
reward_move_interval = 10000  # Rewards change every 10 seconds
time_elapsed = 0
...
# Variables for reward collision
...
# Main game loop
frame_count = 0
FRAMES_PER_STAT = 30  # Statistics are recorded every 30 frames
...
while running:
    ...
    if current_screen == "menu":
        ...
        # Statistics are recorded every 30 frames
        if frame_count % FRAMES_PER_STAT == 0:
            game_stats['time'].append(time_elapsed)
